...
function domains (req, res) {
    ...
    for (let i=0; i<branches.length; i++) {
        ...
        if (bs.length === 1) {
            ...
            console.log("branches => " + bs.length + ", domain => " + domain.length);

            if (domain.length < bs.length) {
                // convert domains to branches,
                const group = [];

                // TODO: we need mark tuple as checked, for that we need to send tupleId from previus phase.
                for (let j=0; j<domain.length; j++) {
                    ...
                    for (let v in d) {
                        ...
                        console.log("dv => " + JSON.stringify(dv));

                        zvs.branches.transform(
                            b, +v,
                            // TODO: domain must be converted to objects.
