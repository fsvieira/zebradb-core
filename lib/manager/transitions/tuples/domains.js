...
function _domains (req, res) {
    ...
    for (let i=0; i<branches.length; i++) {
        ...
        variables = variables.concat(domains);
        ...
        if (bs.length === 1 || variables.length === 0) {
            ...
            for (let b=0; b<bs.length; b++) {
                ...
                // -- debug, print defer table,
                /*
                console.log("--- debug start defer table ---");
                for (let v in defer) {
                    console.log(
                        JSON.stringify(zvs.getObject(branchId, +v)) + " => " + 
                        JSON.stringify(defer[v].map(v => zvs.getObject(queryBranchId, v)))
                    );
                }
                console.log("--- debug end defer table ---");
                */
                ...
                /*
                console.log("--- debug start defer table (2) ---");
                for (let v in defer) {
                    console.log(
                        JSON.stringify(zvs.getObject(branchId, +v)) + " => " + 
                        JSON.stringify(zvs.getObject(queryBranchId, defer[v]))
                    );
                }
                console.log("--- debug end defer table (2) ---");
                */
