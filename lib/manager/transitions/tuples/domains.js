...
function isTupleEqual (zvs, a, b, variables, vtable) {
    console.log("Compare " + utils.toString(zvs.getObject(zvs.branches.root, a), true) 
        + " = " 
        + utils.toString(zvs.getObject(zvs.branches.root, b), true)
    );
...
/*
                ...
                for (let v=0; v<variables.length; v++) {
                    ...
                    if (defer[dataId]) {
                        ...
                        const n = normalize(zvs, queryBranchId, dataId, defer);
                        values.push(n);
                    }
                }
                ...
                for (let v=0; v<variables.length; v++) {
                    ...
                    if (defer[dataId]) {
                        ...
                        vs.push({variable: id, value: {type: 'value', data: __normalize(zvs, queryBranchId, dataId, defer)}});
                    }
                }
