        ...
        if (bs.length === 1 || variables.length === 0) {
            ...
            for (let b=0; b<bs.length; b++) {
                ...
                for (let v=0; v<variables.length; v++) {
                    ...
                    // TODO: normalize types to be the data type.
                    // TODO: handle domain data type ? 
                    if (defer[dataId]) {
                        values.push({group: b, variable: id, value: {type: 'unify', data: defer[dataId]}});
                    }
                    else if (type === 'tuple') {
                            values.push({group: b, variable: id, value: {type: 'unify', data: __normalize(zvs, queryBranchId, dataId, defer)}});
                    }
                    else {
