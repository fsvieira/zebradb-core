...
function normalize (zvs, branchId, id, defer) {
    ...
    if (type === 'tuple') {
        ...
        for (let i=0; i<tupleData.length; i++) {
            ...
            const nID = normalize(zvs, branchId, id, defer);
            ...
            for (let b=0; b<bs.length; b++) {
                ...
                for (let v=0; v<variables.length; v++) {
                    ...
                    if (defer[dataId]) {
                        ...
                        values.push({group: b, variable: id, value: {type: 'unify', data: normalize(zvs, queryBranchId, dataId, defer)}});
                    }
                    else {
                ...
                if (nots && nots.length > 0) {
                    const normalizeNots = nots.map(tupleId => normalize(zvs, queryBranchId, tupleId, defer));
                    normalizeNots.sort();
