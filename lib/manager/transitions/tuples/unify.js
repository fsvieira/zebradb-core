...
function domainXdomain (zvs, branchId, p, q) {
	...
	zvs.update(branchId, gFailId, {data: gDomains});
	...
	if (domain.length === 1) {
		...
		console.log(JSON.stringify(zvs.getObject(branchId, p)));
		console.log(JSON.stringify(zvs.getObject(branchId, q)));
