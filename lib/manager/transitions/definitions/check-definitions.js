    ...
    return function (req, res) {
        ...
        for (let i=definitionsIds.length-1; i>=0; i--) {
            if (!checkDefinition(tmpZVS, tmpZVS.branches.root, definitionsIds[i], match)) {
                console.log("Invalid definition: " + utils.toString(definitions[i]));
                if (failRecover) {
                    ...
                    */
                    events.trigger("error", "Invalid definition: " + utils.toString(definitions[i]) + ", before query: " + utils.toString(query));
                    res.send({});
                    return;
                }
            }
