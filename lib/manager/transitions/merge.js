...
function merge (req, res) {
    const {zvs, negations, events} = req.context;
    function neg ({branchId}) {
    ...
    // console.log("Merge " + mid + " Start!!");

    value.pWhile(function (value, wResolve) {
        if (value.length <= 1) {
            res.send({values: value[0]});
            wResolve(false);
            // console.log("Merge End [1]");
            // console.log("Merge " + mid + " End [1]!!");
            return;
        }
        ...
        Promise.all(p).then(function (v) {
            ...
            if (r.length) {
                ...
                res.send({});
                wResolve(false);
                // console.log("Merge " + mid + " End [3]!!");
            }
        }, function () {
            // TODO: mark branch fail
            res.send({});
            wResolve(false);
            // console.log("Merge " + mid + " End [4]!!");
        });
    });
}
