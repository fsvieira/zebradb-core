...
/*
    order graph:
        - ordered state,
            -> Its a state on ordered list,
        - keep order state,
            -> any state with a path to at least one ordered state is a keep order state.
        - unordered state.
            -> any state that is not a keep state or ordered state.
*/
...
function classifyStates (transitions, orded) {
    const states = {};
    
    getStates(transitions).forEach(function (state) {
        states[state] = {
            orded: orded.indexOf(state) !== -1,
            keepOrder: false,
            unorded: false
        };
    });
...
class Cycles {
    ...
    constructor ({transitions, orded, start}) {
        ...
        classifyStates(transitions, orded);
