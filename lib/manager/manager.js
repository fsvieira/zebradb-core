...
function exists (session) {
	return (branchId, tupleId) => {
		return new Promise((resolve, reject) => {
			...
			/**
			 * TODO:
			 * 	1) If query success with no flop, everything fails.
			 *  2) If query success with a flop:
			 *    a) a flop represents failing branches,
			 *    b) failing branches should not succed, so (1) will never happen.
			 *  3) If query fails with no flop:
			 *    a) then the all query fails, (1) and (3) will never happen.
			 */
			session.internalQuery(queryBranchId, (r, f, b) => {
				const d = session.zvs.getObject(b, session.zvs.data.global("flop"));
				flops = flops.concat(d.data || []);

				if (!queryExists) {
					if (d.data && d.data.length) {
						// if we are able to fail this result, then add to flops,
						flops = flops.concat(d);
					}
					else {
				...
				filterUncheckedTuples: {
					...
					dispatch: value => {
						if (value.tuples.length === 0) {
							/*
							if (value.domainId) {
								// there is no more tuples to be checked outside domains
								// but there can be tuples inside domains that are still 
								// unckecked, so we need to split them. 
								
								return "splitUncheckedDomain";
							}*/
