function Variable (options) {
	...
	this.getValues = function () {
		...
		for (var i in this.share.domain) {
			if (this.share.domain[i].length === l) {
				if (isNaN(+i)) { // TODO: treat all values as strings or keep original types.
					values.push(i);
				}
				else {
					values.push(+i);
	...
	this.tryValues = function (sl, stop, top) {
		stop = stop || [];
		...
		if (stop.indexOf(this) === -1) {
			...
			if (this.getValue() === undefined) {
				...
				d.forEach (function (value) {
					...
					if (self.setValue(value)) {
						...
						self.share.not_equal.forEach (function (v) {
							save = save && v.tryValues(sl, stop.splice(0), false);
						});
				...
				no_values.forEach (function (value) {
					self.setNoValue(value);
				});	
