...
function getZVS () {
    ...
    zvs.action("unify", function (p, q) {
        return unify(p, q, this); // we haven't setup globals to be able to use negation.
    })
...
function multiply (definitions) {
    ...
    for (var i=0; i<definitions.length; i++) {
        p = zvs.add(definitions[i]);
        r = [];
        
        for (var j=i; j<definitions.length; j++) {
            q = zvs.add(definitions[j]);
            r = r.concat(zvs.change("unify", [p, q]) || []);
        }
        
        r = _multiply(zvs, r);

        for (var j=0; j<r.length; j++) {
            q = zvs.getObject(p, r[j]);
...
function transitions (tuple, defs) {
    ...
    zvs.action("unify", function (p, q) {
        return unify(p, q, this, false, true); // we haven't setup globals to be able to use negation.
    });
