...
var unifyOp = require("./unify");
var utils = require("./utils");
var prepare = require("./prepare");

const {mergeConflictHandler} = require("./actions/unify");

// TODO: we need to find a solution to this unify combinations.
function unify (zvs, {branchId: parentBranchId, args: [p, q]}) {
    ...
    return unifyOp(zvs, branchId, p, q)?[branchId]:undefined;
}

function transitionUnify (zvs, {branchId: parentBranchId, args: [p, q]}) {
    const parent = zvs.branches.getRawBranch(parentBranchId);
    ...
    return unifyOp(zvs, branchId, p, q, false, true)?[branchId]:undefined;
}
...
function getZVS () {
    ...
    .action("mergeConflictHandler", function (zvs, branchId, p, q) {
        return unifyOp(zvs, branchId, p, q, this, false);
    });*/
    ...
    for (var i=0; i<definitions.length; i++) {
        ...
        for (var j=i; j<definitions.length; j++) {
            ...
            r = r.concat(/*zvs.change(zvs.branches.root, "unify", [p, q])*/ 
                unify(zvs, {branchId: zvs.branches.root, args: [p, q]}) || []);
        }
...
function transitions (tuple, defs) {
    ...
    for (var i=0; i<defs.length; i++) {
        if (tuple.virtual.code !== defs[i].virtual.code) {
            ...
            r = transitionUnify(zvs, {branchId: zvs.branches.root, args: [p, q]}); // zvs.change(zvs.branches.root, "unify", [p, q]);
