...
function setValue (table, p, value) {
	...
	if (!varP) {
		...
		setValue(table, varP.defered, value);
	}
	else {
		if (varP.notEquals) {
...
function queryTable (p, table, defs, deep, level) {
	if (table) {
		...
		for (var i=0; i<p.tuple.length; i++) {
			...
			if (t) {
				// LOOP point, needs to merge and check table before going further.
				var childs = queryStep(table, t, defs, deep, level);
...
/*
	Query step, 
*/
function queryStep (table, p, defs, deep, level) {
	if (!deep || (level < deep)) {
		var results = [];
...
function query (p, defs, deep) {
	// prepare q,
	p = rename.giveNames(p);// rename.getTupleVariables(p);

	for (var i=0; i<defs.length; i++) {
		/*var q = defs[i];
		q.bound = rename.getTupleVariables(q);*/
		defs[i] = rename.giveNames(defs[i]);
	}
	
	var r = queryStep(undefined, p, defs, deep, 0);
...
function run (defs) {
	...
	return function (q, deep) {
		...
		return query(q, defs, deep);
	};
}

// TODO: export functions to test [mocha], keep them hidden on zebra model.
module.exports = {
	run: run
};
