...
_operators.queryTable = function (table, p, defs, deep, level) {
	if (table) {
		var results = [table];
		...
		for (var i=0; i<p.tuple.length; i++) {
			...
			for (var j=0; j<results.length; j++) {
				...
				if (t) {				
					var childs = _operators.queryDefinitions(table, t, defs);
					if (childs) {
						r = r.concat(childs);
					}
					else {
		...
		for (var i=0; i<p.tuple.length; i++) {
			...
			for (var j=0; j<results.length; j++) {
				...
				if (t) {				
					var childs = _operators.queryTable(table, t, defs);
					if (childs) {
						r = r.concat(childs);
					}
					else {
...
_operators.queryStep = function (table, p, defs, deep, level) {
	if (!deep || (level < deep)) {
		var r = _operators.queryDefinitions(table, p, defs);
		var results = [];
		if (r) {
			for (var i=0; i<r.length; i++) {
				var t = r[i];
				var qt = _operators.queryTable(t, p, defs, deep, level+1);
				if (qt) {
					results = results.concat(qt);
				}
			}
