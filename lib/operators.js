...
_operators.merge = function (tableA, tableB) {
    ...
    if (tableA && tableB) {
    	...
    	tableB = _operators.renameTablesVars(tableB, _operators.getNewNames(varsA, tableB.bound));
    	...
    	tableA = _operators.renameTablesVars(tableA, _operators.getNewNames(varsB, tableA.bound));
		...
		if (tables) {
			...
			for (var i=0; i<tables.length; i++) {
				...
				for (var j=0; j<vars.length; j++) {
					...
					if (t) {
						ts = ts.concat(_operators.getTuples(table, t, ts));
					}
				}
...
require("./logme.js").proxy(_operators, {
	prefix: "operators.",
	out: "logs/operators",
	replace: true,
