...
function query (zvs, success$, {branchId: parentBranchId, args: [q]}) {
    return new Promise(function (resolve, reject) {
        ...
        if (settings.deep && zvs.branches.getLevel(branchId) > settings.deep) {
            zvs.branches.end(branchId, true, "max deep limits exceded!");
            return;
        }
        ...
        if (tuples && tuples.length > 0) {
            ...
            for (var i=0; i<tuples.length; i++) {
                ...
                if (branches.length === 0) {
                    // fail
                    zvs.branches.end(branchId, true, "check fail!");
                    return;
                }
                else {
                    r.push(branches);
            ...
            while (r.length > 1) {
                ...
                if (nr.length === 0) {
                    // fail,
                    zvs.branches.end(branchId, true, "merge fail!");
                    return;
                }
        ...
        if (r.length > 0) {
            ...
            zvs.branches.end(branchId, true, "no results!");
        }
    });
