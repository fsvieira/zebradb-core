...
Writer.prototype.setUnifies = function (i, j) {
	...
	uID = vi.unify || vj.unify;

	if (uI && uJ) {
		unifies = uI.data.concat(uJ.data);
		data = uI;
		delete this.write[vj.unify];
	}
	else if (uI) {
		uI.data.push(j);
		unifies = uI.data;
...
Writer.prototype.defer = function (i, j) {
	...
	if (i !== j) {
		...
		if (v.type === 'variable' && vj.type === 'variable') {
			vj.recursive = v.recursive || vj.recursive;
		}
		...
		if (v.unify) {
			...
			this.removeListenner(i, i);

			if (vu.data.length === 1) {
				this.removeListenner(j, j);
				vj.unify = undefined;
