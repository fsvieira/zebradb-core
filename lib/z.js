...
function queryTuples (q, tuples, defs, len) {
    ...
    for (var t=0; t<tuples.length; t++) {
        console.log("t: " + (t+1) + ", tl: " + tuples.length + ", perc: " + Math.ceil(((t+1) / tuples.length)*100) + "%");
        var rs = [];
        var id = tuples[t];
        ...
        for (var i=0; i < defs.length; i++) {
            ...
            // var DEBUG = utils.table2string(qc.commit(id), undefined, true) + " * " + utils.table2string(qc.commit(q.data.length + dc.start), undefined, true);
            // console.log(DEBUG_COUNT++ + " -- " + DEBUG);
            if (unify(qc, id, q.data.length + dc.start) !== undefined) {
                // console.log(DEBUG_COUNT + " --> " + DEBUG + " = " + utils.table2string(qc.commit(id), undefined, true));
                rs.push(qc);
            }
            /*
            else {
                console.log(DEBUG_COUNT + " --> " + DEBUG + " = FAIL");
            }*/
        }
...
function query (q, defs, rs, len) {
    ...
    console.log("====> QUERY: " + utils.table2string(q, undefined, true));
    ...
    console.log("PLANNER TUPLES:\n" + tuples.map(function (t) {
        return "\t" + utils.table2string(q, t);   
    }).join("\n") + "\n\n");
