...
function queryTuples (q, tuples, defs, len) {
    ...
    for (var t=0; t<tuples.length; t++) {
        ...
        for (var i=0; i < defs.length; i++) {
            ...
            debug = (++debugCounter) + " #> ("+id + ", " + (q.data.length + dc.start) + ") => " + utils.table2string(qc.commit(id), undefined, true) + " ** " + utils.table2string(qc.commit(q.data.length + dc.start), undefined, true) + " = ";


            console.log("--- [QUERYTUPLES A] debug [START] ---\n" + JSON.stringify(qc.snapshot(), null, '\t') + "\n --- [QUERYTUPLES A] debug [END] ---\n");    

            if (unify(qc, id, q.data.length + dc.start) !== undefined) {
                console.log(debug + utils.table2string(qc.commit(), undefined, true));
                rs.push(qc);
            }
            else {
            ...
            console.log("--- [QUERYTUPLES B] debug [START] ---\n" + JSON.stringify(qc.snapshot(), null, '\t') + "\n --- [QUERYTUPLES B] debug [END] ---\n");    
        ...
        if (r.length > 0) {
            ...
            for (var i=0; i<qA.length; i++) {
                for (var j=0; j<qB.length; j++) {
                    debug = (++debugCounter) + " ##> " + utils.table2string(qA[i].commit(), undefined, true) + " U " + utils.table2string(qB[j].commit(), undefined, true) + " = ";

                    console.log("--- [QUERYTUPLES A1] debug [START] ---\n" + JSON.stringify(qA[i].snapshot(), null, '\t') + "\n --- [QUERYTUPLES A1] debug [END] ---\n");    
                    console.log("--- [QUERYTUPLES A1a] debug [START] ---\n" + JSON.stringify(qB[j].snapshot(), null, '\t') + "\n --- [QUERYTUPLES A1a] debug [END] ---\n");    

                    console.log("-----------------------------");        
                    var tuple = qA[i].union(qB[j]);
                
                    if (tuple) {
                        console.log(debug + utils.table2string(tuple.commit(), undefined, true));
                        r.push(tuple);
                    }
                    else {
                        console.log(debug + "FAIL");
                    ...
                    console.log("--- [QUERYTUPLES B2] debug [START] ---\n" + JSON.stringify(qA[i].snapshot(), null, '\t') + "\n --- [QUERYTUPLES A1] debug [END] ---\n");    
                    console.log("--- [QUERYTUPLES A2a] debug [START] ---\n" + JSON.stringify(qB[j].snapshot(), null, '\t') + "\n --- [QUERYTUPLES A1a] debug [END] ---\n");    
...
var debugQueryCounter = 0;
function query (q, defs, rs, len) {
    // q = cleanDuplicates(q);

    console.log("\n\n==== QUERY ====\n" + table2string(q, q.start, true));
    console.log("--- debug [START] ---\n" + JSON.stringify(q, null, '\t') + "\n --- debug [END] ---\n");    
    ...
    if (tuples === undefined) {
        ...
        tuples.forEach(function (t) {
            console.log("\t" + table2string(q, t, true));
        });
    
        var ts = queryTuples(q, tuples, defs, len);
        if (ts) {
            for (var i=0; i<ts.length; i++) {
                console.log((++debugQueryCounter) + "--- [QUERYTUPLES A] debug [START] ---\n" + JSON.stringify(ts[i].snapshot(), null, '\t') + "\n --- [QUERYTUPLES A] debug [END] ---\n");

                var t = ts[i].commit();

                console.log((++debugQueryCounter) + "--- [QUERYTUPLES B] debug [START] ---\n" + JSON.stringify(t, null, '\t') + "\n --- [QUERYTUPLES B] debug [END] ---\n");
...
function prepareDefs (defs) {
    ...
    var g = zutils.graph(defs.definitions || defs);
    // console.log(utils.graph2dot(g));
    return defs;
}
    ...
    this.query = function (q, len) {
        ...
        console.log(
            definitions.map(function (t) {
                return "\t" + table2string(t, undefined, true);
            }).join("\n")
        );
