...
function Run (deep) {
    ...
    flyd.on(function ({branchId, end}) {
        if (end) {
            ...
            const query = zvs.getObject(queryBranchId, zvs.data.global("query"));
            const result = zvs.getObject(branchId, zvs.data.global("query"));
            
            console.log(utils.toString(query, true) + ": " + utils.toString(result, true));
        }
    ...
    if (r.queries) {
        ...
        for (var query in r.queries) {
            ...
            if (branches.length > 0) {
                this.success$({end: this.zvs.getObject(branches[0], this.zvs.data.global("queryBranchId")).data});
            }
