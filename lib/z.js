...
function query (q) {
    ...
    if (q) {
        this.transform(this.zvs.data.global("query"), q);
    }
    else {
        q = this.zvs.data.global("query");
    }

    var settings = this.getObject(this.zvs.data.global("settings"));
    var defs = this.getObject(this.zvs.data.global("definitions")).definitions;
