...
function getDefRank (def) {
    ...
    for (var i=0; i < def.data.length; i++) {
        ...
        if (v.type === 'tuple') {
            for (var j=0; j<v.data.length; j++) {
                var id = v.data[j];
                if (def.data[id].type === 'variable' || def.data[id].type === 'not') {
                    total++;
    ...
    // console.log("total = " + total + ", refs= " + refs.length);
...
function getDefsRank(defs) {
    ...
    for (var i=0; i<defs.length; i++) {
        ...
        if (!rank[t.data.length]) {
            ls.push(t.data.length);
            rank[t.data.length] = rank[t.data.length] || 0;
        }

        if (rank[t.data.length] < getDefRank(defs[i])) {
            // save max rank ...
            rank[t.data.length] = getDefRank(defs[i]);    
        }
    }
...
function noVars (q, i) {
    ...
    if (v.type === 'tuple') {
        for (var j=0; j<v.data.length; j++) {
            if (!noVars(q, v.data[j])) {
                return false;
            }
        }
    }
    else if (v.type === 'not' || v.type === 'variable') {
        return false;
    }
...
function getNoVarTuples (q) {
    var tuples = [];
    
    for (var i=0; i<q.data.length; i++) {
        var v = q.data[i];
        if (!v.check && v.type === 'tuple' && noVars(q, i)) {
            tuples.push(i);
        }
    }
...
function getTuples (q, defs) {
    var tuples = {};
    var rank = false;

    
    var constants = getNoVarTuples(q);
    // return constant tuples.
    
    if (constants.length > 0) {
        console.log("Found " + constants.length + " constants tuples!!");
        return constants;
    }
    ...
    for (var i=0; i<q.data.length; i++) {
        ...
        if (!v.check && v.type === 'tuple') {
            rank = true;
            tuples[v.data.length] = tuples[v.data.length] || [];
            tuples[v.data.length].push(i);
        }
    }
    
    if (rank) {
        var defsRank = getDefsRank(defs);
    
        var l;
        for (var i=0; i<defsRank.length; i++) {
            l = defsRank[i];
    
            if (tuples[defsRank[i]]) {
                break;
            }
        }
...
function query (q, defs, rs, len) {
    ...
    var tuples = strategy.default(q, defs); // getTuples(q, defs);
    
    if (tuples === undefined) {
        return;
    }
    else if (tuples.length > 0) {
        // tuples = tuples.slice(0, 20);
        // shuffle(tuples);
...
function Run (defs) {
    ...
    this.query = function (q, len) {
		...
		qr.forEach(function (t) {
		   console.log("RESULTS = " + (COUNT++) + " ---> " + utils.table2string(t)); 
		});
		...
		if (r) {
		    return r.map(function (t) {
		        console.log("FINAL RESULTS = " + (COUNT++) + " ---> " + utils.table2string(t)); 
