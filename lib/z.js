...
function planner (q, b) {
    ...
    // Filter by no virtuals,
    var results = tuples.filter(function (t) {
        return !b.get(t).virtual;
    });
    
    if (results.length === 0) {
        
        tuples.sort(function (p, q) {
            return b.getObject(p).virtual.vscore < b.getObject(q).virtual.vscore;
        });
    
        var maxScore = b.getObject(tuples[0]).virtual.vscore;
        
        tuples = tuples.filter(function (a) {
            return b.getObject(a).virtual.vscore === maxScore;
        });
    
        // filter non-loop tuples,
        results = tuples.filter(function (t) {
            return !b.get(b.get(t).loop);
        });
        
        results = results.length === 0?tuples:results;
    }    
...
function check (q, defs, b) {
    ...
    var virtual = b.getObject(q).virtual;
...
Run.prototype.multiply = function (r) {
    if (r) {
        ...
        for (var i=0; i<branches.length; i++) {
            ...
            for (var j=i+1; j<branches.length; j++) {
                ...
                ms = this.zvs.merge([branchA, branchB], "mergeConflictHandler");
...
_planner = utils.profileFunc(_planner, "_planner", 0.01);
planner = utils.profileFunc(planner, "planner", 0.01);
negation = utils.profileFunc(negation, "negation", 0.01);
check = utils.profileFunc(check, "check", 0.01);
query = utils.profileFunc(query, "query", 0.01);
definitions = utils.profileFunc(definitions, "definitions", 0.01);

utils.profile(Run, 0.01);
