...
function copyTable (p) {
    return {
        start: p.start,
        data: p.data.map(function (item) {
            if ((item.type === 'unify') || (item.type === 'tuple')) {
                return {
                    type: item.type,
...
//                    console.log((COUNT++) + ": " + utils.table2string(qA[i]) + " * " + utils.table2string(qB[j]) + " = " + utils.table2string(t));
