        ...
        return new Promise(function (resolve, reject) {
            ...
            session.events.on('halt', function () {
                ...
                for (var i=0; i<keys.length; i++) {
                    ...
                    // const functions = session.zvs.getObject(branchQueryId, session.zvs.data.global("queryFunctions")).data;
                    const queryBranch = session.zvs.branches.getBranch(branchQueryId);
                        ...
                        for (var j=0; j<r.branches.length; j++) {
                            ...
                            if (functions.length) {
                                functions.forEach(function (f) {
                                    var qString;
                                    try {
                                        f = safeEval(["query", "result"], f);
                                        qString = f(undefined, session.zvs.getObject(branchId, queryId)).result;
                                    }
                                    catch (e) {
                                        qString = "Error: " + e;
                                    }
