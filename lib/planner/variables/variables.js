        ...
        for (let i in variables) {
            if (variables[i].value) {
                console.log(i + " = " + JSON.stringify(variables[i].value) + " => " + 
                    variables[i].value.data.map(t => utils.toString(match.zvs.getObject(branchId, t)))
                );
            }
...
function matchVariablesValues (
    ...
    if (valueType === 'tuple' && defValueType === 'tuple') {
        const dv = zvs.getData(branchId, value.data);
        const dd = zvs.getData(branchId, defValue.data);

        // TODO: we need first to get all local variables/definition variables, 
        // and then replace before we match,
        // Or: we can re-check the tuples match with variables, on the root 
        // matchVariablesValues returns?
