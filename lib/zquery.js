...
Constant.prototype.getVariables = Ignore.prototype.getVariables = function () {
  // nothing to do, constant as no vars.
  return [];
};


Variable.prototype.getVariables = function (stop) {
	stop = stop || [];
...
Tuple.prototype.getVariables = function (stop) {
	stop = stop || [];

	for (var i=0; i<this.tuple.length; i++) {
		this.tuple[i].getVariables(stop);
	}
...
// TODO: normalize queries and definitions input:
// 1 - accept string with definitions and queries,
// 2 - accept array with query types, and separated them.
Run.prototype.add = function (definitions) {
	if ((typeof definitions) === "string") {
		var results = this.parse(definitions);
		definitions = results.definitions;
		this.queries = this.queries.concat(results.queries);
	}
