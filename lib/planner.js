...
/*
    Remove possible infinity tuples from results.
*/
function treeMatch (zvs, branchId, q, match) {
    const definitionsIds = match.match(q);
    
    const tuple = zvs.getData(branchId, q);
    
    const definitions = definitionsIds.map(id => {
        return {
            id,
            definition: zvs.getData(branchId, id)
        };
    });
...
function planner_4 (zvs, branchId, q) {
    // const tuples = getTuples(zvs, branchId, q);
    ...
    treeMatch(zvs, branchId, q, match);
