    ...
    tuples.forEach(tupleID => {
        ...
        for (let i=definitions.length-1; i>=0; i--) {
            ...
            for (let j=0; j<tupleData.length; j++) {
                ...
                else if (defValueType === 'variable') {
                    const v = localDefVariables[defValueID] = localDefVariables[defValueID] || {};
                    
                    if (valueType === 'variable') {
                        v.variables = v.variables || [];

                        if (v.variables.indexOf(valueID) === -1) {
                            v.variables.push(valueID);
                        }
                    }
                    else if (valueType === 'tuple') {
                        if (v.type) {
                            if (v.type === valueType) {
                                const defs = match.match(branchId, valueID);
                                if (defs) {
                                    v.data = v.data.filter(d => defs.indexOf(d) !== -1);
                            ...
                            v.type = valueType;
                            v.data = match.match(branchId, valueID);
                        }
                    }
                    else {
                        if (v.type) {
                            if (v.type === valueType) {
                                if (v.type !== valueType || v.data !== valueID) {
                                    // contradiction found on this defintions, we need 
                                    // to remove it,
                                    definitions.splice(i, 1);
                                    localVariables = undefined;
