...
const defer = {
    variable: {
        variable: function (variables, zvs, branchId, match, a, b) {
            const {vid: avid, v: av} = getValue(variables, a);
            const {vid: bvid, v: bv} = getValue(variables, b);
            ...
            if (av && bv) {
                ...
                else if (av.type === bv.type) {
                    if (av.type === 'constant') {
                        return av.data === bv.data;
                    }

                    bv.data = bv.data.filter(d => av.data.indexOf(d) !== -1);

                    // defer a to b.
                    av.type = 'defer';
                    av.data = b;

                    return bv.data.length > 0;
                }
        ...
        tuple: function (variables, zvs, branchId, match, a, tuple) {
            const {vid, v: av} = getValue(variables, a);
            ...
            if (av) {
                if (av.type === 'tuple') {
                    ...
                    av.data = av.data.filter(d => definitions.indexOf(d) !== -1);
                    
                    return av.data.length > 0;
                }
            ...
            variables[a] = {
                type: 'tuple',
                data: match.match(branchId, tuple)
            };
            
            return true;
        },
        constant: function (variables, zvs, branchId, match, a, constant) {
            const {vid, v: av} = getValue(variables, a);
            
            a = vid;

            if (av) {
                return av.type === 'constant' && av.data === constant;
            }
    ...
    tuple: {
        variable: function (variables, zvs, branchId, match, a, b) {
            return defer.variable.tuple(variables, zvs, branchId, match, b, a);
        },
        constant: function (variables, zvs, branchId, match, a, b) {
            return defer.variable.constant(variables, zvs, branchId, match, b, a);
        }
    }
};
    ...
    tuples.forEach(tupleID => {
        ...
        for (let i=definitions.length-1; i>=0; i--) {
            ...
            for (let j=0; j<tupleData.length; j++) {
                ...
                const df = defer[defValueType];
                
                if (df) {
                    const f = defer[defValueType][valueType];
                    
                    if (f) {
                        if (!f(localVariables, zvs, branchId, match, defValueID, valueID)) {
                        ...
                        if (defValueType === 'variable') {
                            if (definitionVariables.indexOf(defValueID) === -1) {
                                definitionVariables.push(defValueID);
                            }
                        }
