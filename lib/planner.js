...
function planner (zvs, branchId, q) {
        ...
        if (definitions && definitions.length) {
            ...
            for (var j=0; j<data.length; j++) {
                ...
                if (type === 'tuple') {
                    ...
                    variables[vcode] = variables[vcode] || [];
                    variables[vcode].push(tupleID);
                }
                /* TODO:
    ...
    if (Object.keys(variables).length > 0) {
        // Choose tuples from variables,
        
        r = [];
        for (var variable in variables) {
            const tuples = variables[variable] = variables[variable].map(tupleID => {
                return {
                    tuple: +tupleID,
            ...
            /*
            const m = tuples.reduce((acc, {tuple, definitions}) => acc * definitions.length, 1);
    ...
    return r;
/*
    const matchTuplesR = r;
    const lsort = {};

    for (var i=0; i<matchTuplesR.length; i++) {
        const {tuple, definitions} = matchTuplesR[i];
