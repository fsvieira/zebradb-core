...
function getTuples (q, b, tuples) {
    ...
    if (tuples.indexOf(q) === -1) {
        var d = b.zvs.getData(b.id, q);
    
        if (b.zvs.getData(b.id, d.type) === 'tuple') {
            if (!d.check || !b.zvs.getData(b.id, d.check)) {
                tuples.push(q);
            }
            
            var data = b.zvs.getData(b.id, d.data);
