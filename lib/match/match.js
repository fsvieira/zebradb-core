...
class Match {
    ...
    isLoop (g, t) {
        const tuples = g.tuples || g;
        for (var i in tuples) {
            if (+i === t || this.isLoop(tuples[i], t)) {
                return true;
            }
        }
        ...
        if (from !== undefined) {
            for (var i=0; i<length; i++) {
                ...
                if (type === 'constant') {
                    ...
                    // we just need to match the position,
                    variables.push(i);
                }
            ...
            this.states[from].forEach(d => {
                ...
                r[d] = {tuples: {}, loop: false};

                if (this.g) {
                    variables.forEach(p => {
                        const di = data[p];
                        const type = this.zvs.getData(branchId, this.zvs.getData(branchId, di).type);
                        if (type === 'tuple') {
                            const tuples = this.matchRecursive(branchId, di, cache);
                            if (tuples) {
                                const m = tuples[di];
                                
                                r[d].loop = r[d].loop || Object.keys(m).reduce((s, d) => s || this.g.loops[d], false);
                            }
                            
                            /*r[d].virtual.push({
                                position: p,
                                definitions: Object.keys(m)
                            });*/
                        }
                    });
                }
