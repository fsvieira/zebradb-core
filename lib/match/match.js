...
class Match {
    ...
    constructor () {
        ...
        this.states = {};
        this.start = this.stateIDs.id({tuples: [], final: false});
        this.transitions = {};
    }
    ...
    getStateJoinID (states) {
        if (states.length > 1) {
            ...
            return this.stateIDs.id({tuples, final});
        }
        else {
            return states[0];
        }
    ...
    add (zvs, tupleID, variables) {
        ...
        */
        ...
        // Create state,
        const stateID = this.states.id({tuples: [tupleID], final: true});
        this.states[stateID] = [tupleID];
        
        const tuple = zvs.getObject(zvs.branches.root, tupleID);
        ...
        for (var i=0; i<tuple.data.length; i++) {
            ...
            variables[tuple.data.length][i].symbols.push();
            this.transition(this.start, symbol, stateID);
            this.transition(stateID, symbol, stateID);
        }
    }
    
    addTuples (zvs, tuples) {
        const variables = {};
        tuples.forEach(t => {
           this.add(zvs, t, variables);
        });
    ...
    deterministic (stateID) {
        ...
        for (var from in this.transitions) {
            ...
            for (var symbol in symbols) {
                console.log("tos => " + symbols[symbol].length);
    ...
    match (zvs, tupleID) {
        const tuple = zvs.getObject(zvs.branches.root, tupleID);
        const length = tuple.data.length;
        ...
        if (from !== undefined) {
            ...
            return this.states[from].tuples;
        }
    }
}
