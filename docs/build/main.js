...
async function openPage(page) {
    ...
    var html = await fetch(page.url).then(function (response) {
        return response.text();
    });
...
async function openBlog(db) {
    ...
    for (var i = 0; i < posts.length; i++) {
        var post = posts[i];
        var html = await fetch(post.url).then(function (response) {
            return response.text();
        });

        mainContent.innerHTML += renderPage(post.title, post.author, post.date, html);
    }
...
},{"babel-polyfill":2,"whatwg-fetch":328}],2:[function(require,module,exports){
(function (global){
"use strict";
