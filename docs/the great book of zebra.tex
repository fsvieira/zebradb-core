...
The definition of a \zsystem query computation is abstract and can be implemented in several ways.
In this section we discuss some of them.

\subsection{Lazy Evaluation and Unification specialization}

\paragraph{Lazy evaluation} only evaluates expressions that can be reduced without losing meaning, in this
way we dont need to keep track of constrains.

\paragraph{Unification specialization} uses bound variable information to specialize some rules
of unification that will result on better manage of variables.

The following definitions are unifications specializations:

\paragraph{Two tuples}
\[
    (p_{1} \ldots p_{n}) \unify (q_{1} \ldots q_{n}) = (p_{1} \unify q_{1} \ldots p_{1} \unify q_{n})
\]

Anything else delay to unify.

\paragraph{Let $([v_{1} \ldots v_{i}] p_{0} \ldots p_{n})$ be a tuple and $p$}




\paragraph{Unify Tuples ($\unifytuple$, binary operation)}, will only evalute unify of expressions with tuples, anything else 
